...
int main(int argc, char **argv)
{
	...
	if (argc < 5 || argc % 2 != 1)
	{
		printf("Usage: SkylineBinPackTest binWidth binHeight w_0 h_0 w_1 h_1 w_2 h_2 ... w_n h_n\n");
		...
		printf("Example: SkylineBinPackTest 256 256 30 20 50 20 10 80 90 20\n");
		return 0;
	}
	...
	bin.Init(binWidth, binHeight, false); // Do not use GuillotineBinPack, which is for recovering the losing space.
	...
	for(int i = 3; i < argc; i += 2)
	{
		...
		// Perform the packing.
		SkylineBinPack::LevelChoiceHeuristic heuristic = SkylineBinPack::LevelBottomLeft; // This can be changed individually even for each rectangle packed.
